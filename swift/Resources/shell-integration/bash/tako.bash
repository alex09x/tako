...
if [ -n "$TAKO_BASH_INJECT" ]; then
  ...
  builtin declare __tako_bash_flags="$TAKO_BASH_INJECT"
  builtin unset ENV TAKO_BASH_INJECT

  # Restore an existing ENV that was replaced by the shell integration code.
  if [[ -n "$TAKO_BASH_ENV" ]]; then
    builtin export ENV=$TAKO_BASH_ENV
    builtin unset TAKO_BASH_ENV
  fi
  ...
  if [[ -n "$TAKO_BASH_UNEXPORT_HISTFILE" ]]; then
    builtin export -n HISTFILE
    builtin unset TAKO_BASH_UNEXPORT_HISTFILE
  fi
  ...
  if builtin shopt -q login_shell; then
    if [[ $__tako_bash_flags != *"--noprofile"* ]]; then
      [ -r /etc/profile ] && builtin source "/etc/profile"
      for __tako_rcfile in "$HOME/.bash_profile" "$HOME/.bash_login" "$HOME/.profile"; do
        [ -r "$__tako_rcfile" ] && {
          builtin source "$__tako_rcfile"
          break
        }
      done
    fi
  else
    if [[ $__tako_bash_flags != *"--norc"* ]]; then
      ...
      for __tako_rcfile in /etc/bash.bashrc /etc/bash/bashrc /etc/bashrc; do
        [ -r "$__tako_rcfile" ] && {
          builtin source "$__tako_rcfile"
          break
        }
      done
      if [[ -z "$TAKO_BASH_RCFILE" ]]; then TAKO_BASH_RCFILE="$HOME/.bashrc"; fi
      [ -r "$TAKO_BASH_RCFILE" ] && builtin source "$TAKO_BASH_RCFILE"
    fi
  fi

  builtin unset __tako_rcfile
  builtin unset __tako_bash_flags
  builtin unset TAKO_BASH_RCFILE
fi

# Add Tako binary to PATH if the path feature is enabled
if [[ "$TAKO_SHELL_FEATURES" == *"path"* && -n "$TAKO_BIN_DIR" ]]; then
  if [[ ":$PATH:" != *":$TAKO_BIN_DIR:"* ]]; then
    export PATH="$PATH:$TAKO_BIN_DIR"
  fi
fi

# Sudo
if [[ "$TAKO_SHELL_FEATURES" == *"sudo"* && -n "$TERMINFO" ]]; then
  # Wrap `sudo` command to ensure Tako terminfo is preserved.
  #
...
if [[ "$TAKO_SHELL_FEATURES" == *ssh-* ]]; then
  function ssh() {
    builtin local -a flags
    flags=()
    [[ "$TAKO_SHELL_FEATURES" != *ssh-env* ]] && flags+=(--forward-env=false)
    [[ "$TAKO_SHELL_FEATURES" != *ssh-terminfo* ]] && flags+=(--terminfo=false)
    "$TAKO_BIN_DIR/tako" +ssh "${flags[@]}" -- "$@"
  }
fi
...
_tako_executing=""
_tako_last_reported_cwd=""

function __tako_precmd() {
  local ret="$?"
  if test "$_tako_executing" != "0"; then
    _TAKO_SAVE_PS1="$PS1"
    _TAKO_SAVE_PS2="$PS2"
    ...
    # Cursor
    if [[ "$TAKO_SHELL_FEATURES" == *"cursor"* ]]; then
      builtin local cursor=5  # blinking bar
      [[ "$TAKO_SHELL_FEATURES" == *"cursor:steady"* ]] && cursor=6  # steady bar
    ...
    if [[ "$TAKO_SHELL_FEATURES" == *"title"* ]]; then
      PS1=$PS1'\[\e]2;\w\a\]'
    fi
  fi

  if test "$_tako_executing" != ""; then
  ...
  if [[ "$_tako_last_reported_cwd" != "$PWD" ]]; then
    _tako_last_reported_cwd="$PWD"
...
function __tako_preexec() {
  ...
  if [[ -n $cmd && "$TAKO_SHELL_FEATURES" == *"title"* ]]; then
...
if (( BASH_VERSINFO[0] > 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] >= 4) )); then
  ...
  __tako_hook() {
    ...
    if [[ "$PS0" != *"__tako_preexec_hook"* ]]; then
      if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 3) )); then
        # shellcheck disable=SC2016
        PS0+='${ __tako_preexec_hook; }'
      else
        # shellcheck disable=SC2016
        PS0+='$(__tako_preexec_hook >/dev/tty)'
      fi
    fi
  }
  ...
  # errors from inside __tako_hook itself, but those are all terminal escape
  # sequences and non-actionable.
  #
  # shellcheck disable=SC2128,SC2178,SC2179
  if [[ ";${PROMPT_COMMAND[*]:-};" != *";__tako_hook 2>/dev/null;"* ]]; then
    if [[ -z "${PROMPT_COMMAND[*]}" ]]; then
      if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 1) )); then
        PROMPT_COMMAND=("__tako_hook 2>/dev/null")
      else
        PROMPT_COMMAND="__tako_hook 2>/dev/null"
      fi
  ...
  preexec_functions+=(__tako_preexec)
  precmd_functions+=(__tako_precmd)
fi

# ── Syntax-highlighted `cat`, when bat is installed ──────────────────────────
#
# The terminal itself cannot do this: it sees a byte stream, not files, so it
# knows neither the language nor where one command's output ends. What
# delivers "my code is coloured" is a pager that understands files, and bat is
# that tool.
#
# TAKO_HIGHLIGHT=0, or dropping `highlight` from TAKO_SHELL_FEATURES,
# keeps the plain one.
if [[ "${TAKO_HIGHLIGHT:-1}" != 0 ]] \
  && [[ ";${TAKO_SHELL_FEATURES:-};" == *";highlight;"* ]] \
  && builtin command -v bat >/dev/null 2>&1; then
  cat() {
    # Only colour when a human is looking. The moment stdout is a pipe or a
    # file, `cat` has to stay the byte-exact tool every script assumes it is.
    if [[ ! -t 1 ]] || (( $# == 0 )); then
      builtin command cat "$@"
    else
      builtin command bat --style=plain --paging=never -- "$@"
    fi
  }
fi
