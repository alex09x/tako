...
class AppDelegate: NSObject,
    ...
    @MainActor private lazy var menuShortcutManager = Tako.MenuShortcutManager()
    ...
    private func syncAppearance(config: Tako.Config) {
        NSApplication.shared.appearance = .init(takoConfig: config)
    }

    private func updateAppIcon(from config: Tako.Config) {
        Task.detached {
