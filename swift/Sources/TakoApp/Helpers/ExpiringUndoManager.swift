...
class ExpiringUndoManager: UndoManager {
    ...
    override func removeAllActions() {
        ...
        // Deiniting an ExpiringTarget re-enters this instance to remove itself
        // from `expiringTargets` (see `expire()`); assigning `[] ` directly would
        // deinit the old set's members while that same assignment still holds
        // exclusive access to the property, crashing with an access conflict.
        // Holding the old set until the assignment is done lets those
        // members deinit after the property already points at the new,
        // empty set.
        withExtendedLifetime(expiringTargets) {
            expiringTargets = []
        }
    }
