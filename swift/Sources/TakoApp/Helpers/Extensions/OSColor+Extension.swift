...
#if !DOCK_TILE_PLUGIN
extension OSColor {
    /// Create a color from a Tako color.
    convenience init(tako: tako_config_color_s) {
        let red = Double(tako.r) / 255
        let green = Double(tako.g) / 255
        let blue = Double(tako.b) / 255
