...
extension NSWindow {
    ...
    func addTabbedWindowSafely(
        ...
        var error: NSError?
        let success = TakoAddTabbedWindowSafely(self, child, ordered.rawValue, &error)
        if let error {
            Tako.logger.error("addTabbedWindow failed: \(error.localizedDescription, privacy: .public)")
        }
