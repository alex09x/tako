...
extension NSScreen {
    ...
    var hasDock: Bool {
        let dockAutohide = UserDefaults.tako.persistentDomain(forName: "com.apple.dock")?["autohide"] as? Bool
        return hasDock(dockAutohides: dockAutohide ?? false)
    }

    /// `hasDock` for a given autohide preference, which `hasDock` reads from
    /// the Mac it runs on.
    func hasDock(dockAutohides: Bool) -> Bool {
        // If the dock autohides then we don't have a dock ever.
        if dockAutohides { return false }
