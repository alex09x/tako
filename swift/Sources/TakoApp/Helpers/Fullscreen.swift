...
class NonNativeFullscreen: FullscreenBase, FullscreenStyle {
    ...
    func enter() {
        ...
        // Focus window. select(), not makeKeyAndOrderFront directly: native
        // tabbing is disallowed, so nothing else hides whichever sibling tab
        // was showing before the way AppKit used to.
    ...
    func exit() {
        ...
        if let tabGroup = savedState.tabGroup,
           let tabIndex = savedState.tabGroupIndex,
            ...
            Tako.CustomTabGroup.insert(window, into: tabGroup.windows.first!, at: tabIndex, select: true)
        }
    ...
    private func fullscreenFrame(_ screen: NSScreen) -> NSRect {
        ...
        if !NSApp.presentationOptions.contains(.autoHideMenuBar) &&
            ...
            // if a bug is reported to Tako we can just advise the user to
            // update.
    ...
    class SavedState {
        let screenID: UInt32?
        let tabGroup: Tako.CustomTabGroup?
        ...
        init?(_ window: NSWindow) {
            ...
            self.tabGroupIndex = tabGroup.windows.firstIndex(of: window)
...
extension Notification.Name {
    static let fullscreenDidEnter = Notification.Name("com.tako-core.terminal.fullscreenDidEnter")
    static let fullscreenDidExit = Notification.Name("com.tako-core.terminal.fullscreenDidExit")
}
