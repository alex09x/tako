...
/// Handler for the `send mouse button` AppleScript command defined in `Tako.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="TakoScriptMouseButtonCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(TakoScriptMouseButtonCommand)
class ScriptMouseButtonCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let action: Tako.Input.MouseState
        ...
        let mouseEvent = Tako.Input.MouseButtonEvent(
            action: action,
            button: button.takoButton,
            mods: mods
        )
...
/// Four-character codes matching the `mouse button` enumeration in `Tako.sdef`.
private enum ScriptMouseButtonValue {
    ...
    var takoButton: Tako.Input.MouseButton {
        switch self {
        case .left: .left
        case .right: .right
        case .middle: .middle
