...
/// AppleScript-facing wrapper around a live Tako terminal surface.
///
...
///
/// Mapping from `Tako.sdef`:
/// - `class terminal` -> this class (`@objc(TakoAppleScriptTerminal)`).
...
final class ScriptTerminal: NSObject {
    ...
    weak var surfaceView: Tako.SurfaceView?
...
/// Converts four-character codes from the `split direction` enumeration in `Tako.sdef`
...
enum ScriptSplitDirection {
    ...
    var splitDirection: SplitTree<Tako.SurfaceView>.NewDirection {
        switch self {
        case .right: .right
        case .left: .left
        case .down: .down
