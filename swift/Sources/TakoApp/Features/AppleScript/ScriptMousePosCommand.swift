...
/// Handler for the `send mouse position` AppleScript command defined in `Tako.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="TakoScriptMousePosCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(TakoScriptMousePosCommand)
class ScriptMousePosCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let mousePosEvent = Tako.Input.MousePosEvent(
            x: x,
            y: y,
            mods: mods
        )
