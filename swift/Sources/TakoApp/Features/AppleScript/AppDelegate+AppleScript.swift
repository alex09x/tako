...
// Application-level Cocoa scripting hooks for the Tako AppleScript dictionary.
//
...
// the runtime object behind the `application` class in `Tako.sdef`.
...
extension NSApplication {
    ...
    /// the frontmost logical Tako window.
    @objc(frontWindow)
...
extension NSApplication {
    ...
    func handleNewSurfaceConfigurationScriptCommand(_ command: NSScriptCommand) -> NSDictionary? {
        ...
        do {
            let configuration = try Tako.SurfaceConfiguration(
    ...
    func handleNewWindowScriptCommand(_ command: NSScriptCommand) -> ScriptWindow? {
        ...
        guard let appDelegate = delegate as? AppDelegate else {
            ...
            command.scriptErrorString = "Tako app delegate is unavailable."
            return nil
        }

        let baseConfig: Tako.SurfaceConfiguration?
        if let scriptRecord = command.evaluatedArguments?["configuration"] as? NSDictionary {
            do {
                baseConfig = try Tako.SurfaceConfiguration(scriptRecord: scriptRecord)
            } catch {
                command.scriptErrorNumber = errAECoercionFail
                command.scriptErrorString = error.localizedDescription
                return nil
    ...
    /// If no window is provided, this uses the preferred parent window.
    ///
...
@MainActor
extension NSApplication {
    /// What every scripting object/command asks to decide whether it should
    /// act. `macosAppleScript` is currently hardcoded to `true`
    /// (`Tako+Config.swift`), so the real check can never produce `false` --
    /// tests substitute this closure to exercise the "disabled" branches that
    /// every AppleScript entry point in this feature guards behind it.
    struct AppleScriptAvailability {
        var isEnabled: () -> Bool = {
            guard let appDelegate = NSApp.delegate as? AppDelegate else { return true }
            return appDelegate.tako.config.macosAppleScript
        }
    }

    static var appleScriptAvailability = AppleScriptAvailability()

    /// Whether Tako should currently accept AppleScript interactions.
    var isAppleScriptEnabled: Bool {
        NSApplication.appleScriptAvailability.isEnabled()
    }
    ...
    fileprivate var allSurfaceViews: [Tako.SurfaceView] {
