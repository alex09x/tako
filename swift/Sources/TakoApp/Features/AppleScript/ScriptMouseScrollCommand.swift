...
/// Handler for the `send mouse scroll` AppleScript command defined in `Tako.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="TakoScriptMouseScrollCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(TakoScriptMouseScrollCommand)
class ScriptMouseScrollCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let momentum: Tako.Input.Momentum
        ...
        let scrollEvent = Tako.Input.MouseScrollEvent(
            x: x,
            y: y,
