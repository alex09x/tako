...
final class ScriptTab: NSObject {
    ...
    func handleSelectTab(_ command: NSScriptCommand) -> Any? {
        ...
        // `makeKeyAndOrderFront` alone doesn't hide whichever tab was
        // showing before -- native tabbing is disallowed, so nothing else
        // does that automatically the way it did when AppKit owned tabs.
        Tako.CustomTabGroup.group(for: tabContainerWindow).select(tabContainerWindow)
        return nil
    }
