...
class TransparentTitlebarTerminalWindow: TerminalWindow {
    ...
    private var lastSurfaceConfig: Tako.SurfaceView.DerivedConfig?
    ...
    /// The macOS 13-to-15 and macOS 26+ appearance paths below are both live
    /// production code, but only one of them is ever reachable via `#available`
    /// on any given OS. A test host only ever runs one real OS at a time, so
    /// tests substitute this to exercise both branches.
    struct System {
    ...
    static var appearanceSystem = System()
    ...
    override func update() {
        ...
        if !Self.appearanceSystem.isMacOS26OrLater() {
    ...
    private func syncAppearanceTahoe(_ surfaceConfig: Tako.SurfaceView.DerivedConfig) {
    ...
    private func syncAppearanceVentura(_ surfaceConfig: Tako.SurfaceView.DerivedConfig) {
