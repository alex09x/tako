...
/// A base class for windows that can contain Tako windows. This base class implements
/// the bare minimum functionality that every terminal window in Tako should implement.
///
...
class BaseTerminalController: NSWindowController,
    ...
    private var appliedColorScheme: tako_color_scheme_e?
    ...
    init(_ tako: Tako.App,
        ...
        // Initialize our initial surface. A surface is given the app, not its
        // C handle: through the handle it had no config -- no scrollback
        // limit, no Option as Alt, no reloads -- in every real window.
        self.surfaceTree = tree ?? .init(view: Tako.SurfaceView(tako, baseConfig: base))
        // A restored tree was decoded without an app; didSet does not run
        // from an initializer.
    ...
    /// Update all surfaces with the focus state. This ensures that libtako has an accurate view about
    ...
    private func findNextFocusTargetAfterClosing(node: SplitTree<Tako.SurfaceView>.Node) -> Tako.SurfaceView? {
    ...
    @objc private func takoConfigDidChangeBase(_ notification: Notification) {
    ...
    @objc private func takoCommandPaletteDidToggle(_ notification: Notification) {
        guard let surfaceView = notification.object as? Tako.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        toggleCommandPalette(nil)
    }

    @objc private func takoMaximizeDidToggle(_ notification: Notification) {
        guard let window else { return }
        guard let surfaceView = notification.object as? Tako.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        window.zoom(nil)
    ...
    @objc private func takoDidCloseSurface(_ notification: Notification) {
    ...
    @objc private func takoDidNewSplit(_ notification: Notification) {
    ...
    @objc private func takoDidEqualizeSplits(_ notification: Notification) {
        guard let target = notification.object as? Tako.SurfaceView else { return }
    ...
    @objc private func takoDidFocusSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? Tako.SurfaceView else { return }
    ...
    @objc private func takoDidToggleSplitZoom(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? Tako.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
    ...
    @objc private func takoDidResizeSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? Tako.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
    ...
    @objc private func takoDidPresentTerminal(_ notification: Notification) {
        guard let target = notification.object as? Tako.SurfaceView else { return }
        guard surfaceTree.contains(target) else { return }
    ...
    @objc private func takoSurfaceDragEndedNoTarget(_ notification: Notification) {
        guard let target = notification.object as? Tako.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
    ...
    private func splitDidResize(node: SplitTree<Tako.SurfaceView>.Node, to newRatio: Double) {
    ...
    func toggleBackgroundOpacity() {
        ...
        guard tako.config.backgroundOpacity < 1 else { return }
