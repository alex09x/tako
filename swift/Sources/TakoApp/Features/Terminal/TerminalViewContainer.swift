...
///
/// The class and the two properties below are `open`, not upstream's plain
/// `internal`: upstream's own TakoTests target shares one Xcode project
/// with this file, so `@testable import` alone lets it subclass and
/// override internal members there. Here `TakoTests` is a genuinely
/// separate SPM module (see swift/Package.swift), and `-enable-testing`
/// only widens `internal` to look like `public` for *access*, not `open` --
/// TerminalViewContainerTests.swift's `MockTerminalViewContainer` still
/// needs the real thing to enable overriding.
open class TerminalViewContainer: NSView {
    ...
    open var windowThemeFrameView: NSView? {
        window?.contentView?.superview
    }

    open var windowCornerRadius: CGFloat? {
    ...
    func takoConfigDidChange(_ config: Tako.Config, preferredBackgroundColor: NSColor?) {
...
extension TerminalViewContainer {
    ...
    struct DerivedConfig: Equatable {
        ...
        init?(config: Tako.Config, preferredBackgroundColor: NSColor?, cornerRadius: CGFloat?) {
