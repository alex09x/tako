...
protocol TerminalRestorable: Codable {
    ...
    var baseConfig: Tako.SurfaceConfiguration? { get }
}
...
extension TerminalRestorable {
    ...
    var baseConfig: Tako.SurfaceConfiguration? { nil }
...
final class TerminalRestorableState: TerminalRestorable {
    ...
    var surfaceTree: SplitTree<Tako.SurfaceView> {
    ...
    private let internalState: InternalState<Tako.SurfaceView>
    ...
    init(from decoder: any Decoder) throws {
        self.internalState = try InternalState<Tako.SurfaceView>(from: decoder)
    }
...
class TerminalWindowRestoration: NSObject, NSWindowRestoration {
    /// What restoring a window needs to build a controller with a loaded
    /// `NSWindow`. Tests substitute it: `Terminal.xib` is excluded from
    /// this SwiftPM test target, so the real controller's `.window` always
    /// fails to load lazily here, which would make the success path below
    /// permanently unreachable from a test host.
    struct System {
        var makeController: (Tako.App, SplitTree<Tako.SurfaceView>) -> (TerminalController, NSWindow)? = { app, tree in
    ...
    static func restoreWindow(
        ...
        // can be found for events from libtako. This uses the low-level
        // createWindow so that AppKit can place the window wherever it should
        // be.
        guard let (c, window) = system.makeController(appDelegate.tako, state.surfaceTree) else {
        ...
        if let focusedStr = state.focusedSurface {
            var foundView: Tako.SurfaceView?
    ...
    private static func restoreFocus(to: Tako.SurfaceView, inWindow: NSWindow, attempts: Int = 0) {
        ...
        DispatchQueue.main.asyncAfter(deadline: after) {
            ...
            // prevents a bug where sometimes on restore the windows
