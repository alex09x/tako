...
enum QuickTerminalPosition: String {
    ...
    func conflictsWithDock(on screen: NSScreen) -> Bool {
        conflictsWithDock(screenHasDock: screen.hasDock, orientation: Dock.orientation)
    }

    /// `conflictsWithDock(on:)` for a given dock, which that reads from the
    /// Mac it runs on.
    func conflictsWithDock(screenHasDock: @autoclosure () -> Bool, orientation: @autoclosure () -> DockOrientation?) -> Bool {
        // Screen must have a dock for it to conflict
        guard screenHasDock() else { return false }

        // Get the dock orientation for this screen
        guard let orientation = orientation() else { return false }
