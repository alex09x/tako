...
/// Represents the Tako `quick-terminal-size` configuration. See the documentation for
...
struct QuickTerminalSize {
    ...
    init(from cStruct: tako_config_quick_terminal_size_s) {
    ...
    enum Size {
        ...
        init?(from cStruct: tako_quick_terminal_size_s) {
            switch cStruct.tag {
            case TAKO_QUICK_TERMINAL_SIZE_NONE:
                return nil
            case TAKO_QUICK_TERMINAL_SIZE_PERCENTAGE:
                self = .percentage(cStruct.value.percentage)
            case TAKO_QUICK_TERMINAL_SIZE_PIXELS:
