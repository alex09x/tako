...
struct ConfigTests {
    ...
    @Test func macosTitlebarStyleDefaultsToTabs() throws {
        let config = try TemporaryConfig("")
        #expect(config.macosTitlebarStyle == .tabs)
    }

    @Test(arguments: [
        ("native", Tako.Config.MacOSTitlebarStyle.native),
        ("transparent", Tako.Config.MacOSTitlebarStyle.transparent),
        ("tabs", Tako.Config.MacOSTitlebarStyle.tabs),
        ("hidden", Tako.Config.MacOSTitlebarStyle.hidden),
    ])
    func macosTitlebarStyleValues(raw: String, expected: Tako.Config.MacOSTitlebarStyle) throws {
    ...
    @Test(arguments: [
        ("always", Tako.Config.ResizeOverlay.always),
        ("never", Tako.Config.ResizeOverlay.never),
        ("after-first", Tako.Config.ResizeOverlay.after_first),
    ])
    func resizeOverlayValues(raw: String, expected: Tako.Config.ResizeOverlay) throws {
